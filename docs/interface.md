# Agent SDK 网关接口文档

版本：v1.0（对应 LinkSoul AgentSDK v1.4.0 线上协议）

## 1. 概述

机器人本体（网关）作为 **WebSocket 服务端**，Agent 作为 **客户端** 接入。网关职责：

- 采集机器人麦克风音频，经 VAD 切分后推送 Agent
- 接收 Agent 回传的 ASR 文本 / LLM 增量文本 / TTS 音频，驱动机器人扬声器播放
- 维护机器人上下线状态与技能指令通道

```
┌──────────────┐  WebSocket (JSON 文本帧)  ┌──────────────┐
│   机器人网关   │ ◄──────────────────────► │  Agent 客户端 │
│ (Unity/真机)  │   16k PCM base64 内嵌      │ (你的服务)   │
└──────────────┘                           └──────────────┘
```

术语约定：**下行** = 网关 → Agent，**上行** = Agent → 网关。

## 2. 连接与鉴权

### 2.1 端点

| 项 | 值 |
|---|---|
| URL | `ws://<robot-host>:9002/api/V1/open-portal/app/wss/agent-sdk` |
| 协议 | WebSocket（RFC 6455），消息为 JSON 文本帧 |
| 并发会话 | 1（已有连接未释放时新连接被拒） |

> 路径参与签名校验，必须与上面完全一致（含大小写）。

### 2.2 鉴权（HTTP Upgrade 阶段）

握手即认证，无连接后 auth 应答消息。请求头：

| Header | 说明 |
|---|---|
| `X-App-Id` | 应用 ID |
| `X-App-Key` | 应用 Key |
| `X-Timestamp` | 毫秒 Unix 时间戳（±5 分钟内有效） |
| `X-Nonce` | 随机串，每次连接唯一 |
| `X-Signature` | 签名，见下 |
| `X-Callback-Types` | 回调类型 JSON 数组，主路径填 `["audio2tts"]` |

签名算法（HMAC-SHA256，小写 hex）：

```python
import hashlib, hmac

payload = "GET\n" + path + "\n" + ts + "\n" + nonce
signature = hmac.new(app_secret.encode(), payload.encode(),
                     hashlib.sha256).hexdigest()
```

### 2.3 握手响应码

| HTTP 状态 | 含义 |
|---|---|
| `101` | 成功，升级为 WebSocket |
| `400` | 非 WebSocket 升级请求 |
| `401` | 签名/凭证/时间戳校验失败 |
| `404` | 路径不匹配 |
| `503` | 已有会话占用（单会话限制） |

> 交付 exe 默认宽松模式（不校验签名）。开启严格鉴权见 [../exe/README.md](../exe/README.md) 修改配置。

### 2.4 会话生命周期

- 连接成功后网关**立即下发** `robot_state.sync`（§4.1）
- 断线后 Agent 应自动重连（建议间隔 3s），机器人以最新 sync 为准
- 机器人状态周期性推送见 §4.5

## 3. 消息信封

所有消息为 JSON 对象，公共字段：

| 字段 | 方向 | 说明 |
|---|---|---|
| `type` | 双向 | 消息类型，命名空间 `agentsdk.*` |
| `agentId` | 双向 | Agent 应用 ID，网关下发值原样回传 |
| `agentMode` | 上行 | Agent 工作模式（如 `passive`） |
| `robotCid` / `cid` | 双向 | 机器人会话 ID，原样回传 |
| `eventId` | 双向 | 轮次 ID：一次 commit 的轮次内所有回包共用 |
| `itemId` | 双向 | 条目 ID：本轮 ASR/LLM/TTS 条目共用（部分消息可选） |

**核心约定：`agentId` / `robotCid` / `eventId` / `itemId` 必须原样使用网关下发值**，网关按这些字段路由与关联。

## 4. 下行消息（网关 → Agent）

### 4.1 `agentsdk.robot_state.sync` — 机器人上下线

连接建立后第一帧。

```json
{
  "type": "agentsdk.robot_state.sync",
  "agentId": "demo-app",
  "state": "online",
  "callbackType": "audio2tts",
  "agentMeta": {}
}
```

`agentMeta` 为机器人能力元信息。Agent 收到此帧后可主动下发开场播报（`llm_response` + `tts_response`，不依赖对话轮次）。

### 4.2 `agentsdk.audio_request.start` — 语音流开始

VAD 检测到用户开始说话。`audio2tts` 模式携带 `itemId` 及信封字段。

### 4.3 `agentsdk.audio_request.append` — 语音流中间帧

```json
{
  "type": "agentsdk.audio_request.append",
  "itemId": "item-...",
  "audio": "<base64 PCM>",
  "audioLen": 3200
}
```

音频格式：16kHz / 16bit / mono，约 100ms/帧。

### 4.4 `agentsdk.audio_request.commit` — 语音流结束

用户静音超时或达到最长说话时长（§7）。收到此帧即一轮完整语音就绪，Agent 开始处理并回包（§8 时序）。

### 4.5 `agentsdk.state_request.meta` — 状态推送

```json
{ "type": "agentsdk.state_request.meta",
  "stateName": "power", "stateValue": "ok" }
```

### 4.6 `agentsdk.skill_response.state` — 技能状态回报（v1 仿真扩展）

```json
{
  "type": "agentsdk.skill_response.state",
  "skillName": "walk",
  "state": "done",
  "detail": ""
}
```

| state | 含义 |
|---|---|
| `running` | 已接受并开始执行 |
| `done` | 执行完成（手势播完 / 运动站稳 / 表情已设置） |
| `failed` | 未知技能名 / 无对应执行器 |

技能下发是 fire-and-forget；状态回报为仿真扩展帧，Agent 可据此感知动作完成时机（如"走到跟前再说话"），真机对接时可忽略。

## 5. 上行消息（Agent → 网关）

均需携带 §3 信封字段。

### 5.1 ASR 识别结果

| type | 字段 | 说明 |
|---|---|---|
| `agentsdk.asr_response.middle` | `text` | 中间识别结果（可多次） |
| `agentsdk.asr_response.final` | `text` | 最终识别结果（每轮一次） |

### 5.2 LLM 回复（流式）

| type | 字段 | 说明 |
|---|---|---|
| `agentsdk.llm_response.item.delta` | `itemId`, `text` | **文本增量**（非全量），网关逐段累积 |
| `agentsdk.llm_response.item.done` | `itemId` | 本条 LLM 内容结束 |
| `agentsdk.llm_response.done` | — | 本轮 LLM 处理结束 |

### 5.3 TTS 音频（流式）

| type | 字段 | 说明 |
|---|---|---|
| `agentsdk.tts_response.item.delta` | `itemId`, `audio`, `audioLen` | PCM 分段，边收边播 |
| `agentsdk.tts_response.item.done` | `itemId` | 本条音频结束 |
| `agentsdk.tts_response.done` | — | 本轮 TTS 结束，网关复位状态 |

`audio` 为 base64(16kHz / 16bit / mono PCM)。网关收到第一段即开始播放，后续分段自动排队，无需等待合成完成。建议按 200ms/包分帧。

### 5.4 技能指令 `agentsdk.xlm_response.skill`

```json
{
  "type": "agentsdk.xlm_response.skill",
  "itemId": "item-...",
  "skillType": "gesture",
  "skillName": "wave_hands",
  "skillParam": {}
}
```

技能清单（`skillType` / `skillName` / `skillParam`）：

| skillType | skillName | skillParam | 说明 |
|---|---|---|---|
| gesture | `wave_hands` | — | 挥手（右臂举起摆动） |
| gesture | `open_arms` | — | 张开双臂（欢迎姿势） |
| movement | `walk` | `{"distanceM": 1.0}` | 前进指定米数（0.2~5） |
| movement | `turn` | `{"angleDeg": 90}` | 原地转角（右转为正） |
| movement | `stop` | — | 立即停止 |
| emotion | `happy` / `sad` / `surprised` / `angry` / `love` / `neutral` | `{"durationMs": 3000}` | 头部表情屏 |

执行行为：

- 手势由程序化关节轨迹驱动（肩/肘/腕/头），与步态无冲突，可边走边做
- 运动指令在步态支撑相位切换、结束在支撑相位归零
- 表情屏同时联动 TTS 播放能量做口型张合
- 未知 `skillName` 回报 `failed`，不影响语音链路
- 示例项目将上表注册为 LLM 的 `robot_skill` 工具，由 function calling 自动触发

### 5.5 打断指令 `agentsdk.xlm_response.interrupt`

```json
{
  "type": "agentsdk.xlm_response.interrupt",
  "interruptType": "chat",
  "interruptTips": "好的，请说"
}
```

打断后网关停播 TTS 并中止当前运动/手势。

### 5.6 错误上报 `agentsdk.error`

```json
{
  "type": "agentsdk.error",
  "errorCode": 3201,
  "errorMsg": "llm failed: ..."
}
```

| code | 阶段 |
|---|---|
| 3101 / 3102 | ASR 失败 / 空结果 |
| 3201 / 3202 | LLM 失败 / 空回复 |
| 3301 | TTS 失败 |

## 6. 兼容性

- 网关对 `llm_response.item.done`、`vlm_*`、`greet_*`、`xlm_response.control` 等类型**接收但忽略**（v1 范围外，日志可见 `gw ignore`），不影响链路
- 未知 `type` 同样忽略，向前兼容
- Agent 侧心跳无强制要求，依赖 TCP 层保活

## 7. 音频与 VAD 参数

| 项 | 值 |
|---|---|
| 采样率 / 位深 / 声道 | 16000 Hz / 16 bit / 单声道 |
| 上行分帧 | 约 100 ms/帧（1600 samples = 3200 bytes） |
| 传输编码 | base64 内嵌 JSON `audio` 字段 |
| VAD 起说话阈值（RMS） | 0.02 |
| VAD 停止阈值（RMS） | 0.008 |
| 静音判定 | 600 ms |
| 最长单轮语音 | 15000 ms（强制 commit） |
| 半双工 | TTS 播放期间 VAD 冻结，避免自采集 |

## 8. 一轮对话时序

```
机器人(网关)                         Agent
    │  robot_state.sync(online)  ──► │  连接后立即
    │  audio_request.start      ──►  │  VAD 开始
    │  audio_request.append ×N  ──►  │  语音流（100ms/帧）
    │  audio_request.commit     ──►  │  一轮语音就绪
    │                                │  ┌─ ASR（可流式回 middle）
    │  ◄── asr_response.final        │  ├─ LLM 流式
    │  ◄── llm_response.item.delta×N │  │   （每段立即下发）
    │  ◄── llm_response.item.done    │  ├─ 凑句即送 TTS
    │  ◄── llm_response.done         │  │
    │  ◄── tts_response.item.delta×N │  ├─ TTS PCM 分段下发
    │      （收到即播放）             │  │
    │  ◄── tts_response.item.done    │  └─ 剩余文本收尾
    │  ◄── tts_response.done         │
    │  ◄── xlm_response.skill       │  （function calling 触发动作时）
    │  skill_response.state ──►      │  running → done/failed（仿真扩展）
    │  （本轮复位，等待下一轮）        │
```

**流式要点**：LLM delta 与 TTS 分段无需等全部生成完成——首句 TTS 在 LLM 仍在生成后续内容时即可开始播放，实测首包音频延迟 < 2.5s。
